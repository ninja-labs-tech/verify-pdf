...
describe('Test verification', () => {
  ...
  Object.keys(pdfSamples)
    .forEach((sampleName) => {
      ...
      } else if (pdfSamples[sampleName].length) {
        it(`expects sample: '${sampleName}' to be ${!pdfSamples[sampleName].verified && 'not'} valid`, () => {
          const verifyResult = verifyPDF(signedPdfBuffer);
          for (let i = 0; i < pdfSamples[sampleName].length; i += 1) {
            expect(verifyResult[i].verified).toBe(pdfSamples[sampleName][i].verified);
            expect(verifyResult[i].integrity).toBe(pdfSamples[sampleName][i].integrity);
            expect(verifyResult[i].authenticity).toBe(pdfSamples[sampleName][i].authenticity);
            expect(verifyResult[i].expired).toBe(pdfSamples[sampleName][i].expired);
          }
        });
      } else {
        it(`expects sample: '${sampleName}' to be ${!pdfSamples[sampleName].verified && 'not'} valid`, () => {
