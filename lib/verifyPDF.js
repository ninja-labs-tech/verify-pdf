...
module.exports = (pdfBuffer) => {
  ...
  try {
    ...
    const dataDigest = crypto.createHash(hashAlgorithm)
      .update(signedData)
      .digest();
    const integrityVerified = dataDigest.toString('latin1') === attrDigest;
    const sortedCerts = sortCertificateChain(certificates);
    const parsedCerts = extractCertificatesDetails(sortedCerts);
    const authenticityVerified = authenticateSignature(sortedCerts);
    return ({
      verified: integrityVerified && authenticityVerified,
