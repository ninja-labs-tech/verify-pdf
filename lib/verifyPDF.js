...
module.exports = (pdf) => {
  ...
  try {
    ...
    for (let i = 0; i < signedData.length; i += 1) {
      signatures.push(verify(signatureStr[i], signedData[i], signatureMeta[i]))
    };

    return {
      verified: !signatures.some(o => o.verified === false),
      authenticity: !signatures.some(o => o.authenticity === false),
      integrity: !signatures.some(o => o.integrity === false),
