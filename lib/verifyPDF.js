...
module.exports = (pdfBuffer) => {
  ...
  try {
    ...
    const hashAlgorithm = forge.pki.oids[hashAlgorithmOid];
    ...
    const digest = forge.md[hashAlgorithm.toLowerCase()]
      .create()
      .update(forge.asn1.toDer(set).data)
      .digest();
    const validAuthenticatedAttributes = clientCertificate.publicKey.verify(digest.data, sig);
    ...
    const dataDigest = forge.md[hashAlgorithm.toLowerCase()]
      .create()
